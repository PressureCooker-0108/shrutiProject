...
export default function DrinkBuilder({ customization, onChangeCustomization }: DrinkBuilderProps) {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      let colorStyle = "#2C2421";
      const hasMilk = current.milk !== "None";
      switch (current.base) {
        case "Matcha Green":
          colorStyle = hasMilk ? "#8CAE93" : "#2B4D3D";
          break;
        case "Caramel Gold":
          colorStyle = hasMilk ? "#C19A6C" : "#916F38";
          break;
        case "Dragonfruit Refresher":
          colorStyle = hasMilk ? "#E46F9E" : "#E0115F";
          break;
        case "Cold Brew":
        default:
          colorStyle = hasMilk ? "#765C4E" : "#2C2421";
      }
      ...
      // Adjust milk blend transparency
      if (current.milk === "None") {
        liquidMat.opacity = 0.95;
      } else {
        liquidMat.opacity = 0.85;
      }
  ...
  const getLiquidColor = () => {
    ...
    switch (customization.base) {
      case "Matcha Green":
        return hasMilk ? "#8CAE93" : "#2B4D3D";
      case "Caramel Gold":
        return hasMilk ? "#C19A6C" : "#916F38";
      case "Dragonfruit Refresher":
        return hasMilk ? "#E46F9E" : "#E0115F";
      case "Cold Brew":
      default:
        return hasMilk ? "#765C4E" : "#2C2421";
    }
  };
  ...
  return (
    <section id="builder" className="py-16 md:py-24 px-4 md:px-8 max-w-7xl mx-auto space-y-12">
      ...
      <div className="space-y-2 border-b border-[#2C2421]/5 pb-6">
        <div className="flex items-center gap-2 text-xs font-bold text-[#2B4D3D] tracking-wider uppercase">
        ...
        <p className="text-sm md:text-base text-[#2C2421]/75 max-w-xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="rounded-[32px] p-8 bg-[#FAF8F5] border border-[#2C2421]/5 shadow-sm space-y-4">
            <span className="text-[10px] uppercase font-bold text-[#C0783E] tracking-wider">
              {currentBase.category}
            </span>
            <h3 className="text-4xl font-display font-extrabold text-[#2C2421] leading-[1.1] tracking-tight">
              {customization.topping === "Cold Foam" ? "Matcha Foam " : ""}
              {customization.base}
            </h3>
            <p className="text-xs md:text-sm text-[#2C2421]/75 leading-relaxed">
          ...
          <div className="rounded-[32px] p-8 bg-[#FAF8F5] border border-[#2C2421]/5 shadow-sm space-y-4">
            <div className="flex items-center gap-2 text-[#C0783E]">
              <Sparkles size={16} />
            ...
            <ul className="space-y-2 text-xs font-medium text-[#2C2421]/80">
              <li className="flex items-center gap-2">
                <span className="w-1.5 h-1.5 rounded-full bg-[#2B4D3D]" />
                <span>Size: <strong className="text-[#2C2421]">{customization.size}</strong></span>
              </li>
              <li className="flex items-center gap-2">
                <span className="w-1.5 h-1.5 rounded-full bg-[#2B4D3D]" />
                <span>Milk: <strong className="text-[#2C2421]">{customization.milk}</strong></span>
              </li>
              <li className="flex items-center gap-2">
                <span className="w-1.5 h-1.5 rounded-full bg-[#2B4D3D]" />
                <span>Sweetener: <strong className="text-[#2C2421]">{customization.sweetener}</strong></span>
              </li>
              <li className="flex items-center gap-2">
                <span className="w-1.5 h-1.5 rounded-full bg-[#2B4D3D]" />
                <span>Topping: <strong className="text-[#2C2421]">{customization.topping}</strong></span>
              </li>
              <li className="flex items-center gap-2">
                <span className="w-1.5 h-1.5 rounded-full bg-[#2B4D3D]" />
                <span>Ice: <strong className="text-[#2C2421]">{customization.ice}</strong></span>
              </li>
            </ul>
          </div>

          {/* Large Starbucks Dark Green Add to Order panel (Bottom Left placement) */}
          <div className="rounded-[32px] p-8 bg-[#2B4D3D] text-[#F4F0EB] shadow-md space-y-4 relative overflow-hidden">
            {/* Ambient gold glow */}
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-[11px] text-[#F4F0EB]/70 font-medium mt-1">
                  Customized {customization.size}
                </p>
              </div>
              
              <div className="px-4 py-2 rounded-full bg-[#FAF8F5]/10 border border-[#FAF8F5]/10 text-base font-extrabold text-[#FAF8F5]">
                ${price}
              </div>
            </div>
            ...
            <AnimatePresence mode="wait">
              {ordered ? (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: -10 }}
                  className="w-full py-4 rounded-2xl bg-[#FAF8F5] text-[#2B4D3D] flex items-center justify-center gap-2 text-sm font-bold shadow-sm"
                >
                  <Check size={16} />
                ...
                <motion.button
                  ...
                  className="w-full py-4 rounded-2xl bg-[#FAF8F5] text-[#2B4D3D] flex items-center justify-center gap-2 text-sm font-bold transition-custom hover-scale shadow-sm cursor-pointer hover:bg-[#FAF8F5]/90"
                >
                  <ShoppingBag size={16} />
                  <span>Checkout</span>
                </motion.button>
        ...
        <div className="lg:col-span-4 w-full flex justify-center">
          <div className="w-full rounded-[40px] p-6 bg-[#FAF6F2] border border-[#2C2421]/5 shadow-sm flex flex-col items-center relative min-h-[460px] justify-center overflow-hidden">
            ...
            <div className="absolute bottom-4 text-center">
              <span className="text-[9px] font-bold tracking-widest text-[#2C2421]/50 uppercase select-none pointer-events-none">
        ...
        <div className="lg:col-span-4 space-y-4">
          ...
          <div className="rounded-[24px] bg-[#FAF8F5] border border-[#2C2421]/5 shadow-sm overflow-hidden transition-all duration-300">
            <button
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-[#2C2421]/60 mt-1 font-semibold uppercase tracking-wider">
              ...
              <ChevronDown
                size={18}
                className={`text-[#2C2421]/40 transition-transform duration-300 ${
                  expandedSection === "size" ? "rotate-180 text-[#2B4D3D]" : ""
                }`}
              />
            </button>
            ...
            <AnimatePresence initial={false}>
              {expandedSection === "size" && (
                <motion.div
                  ...
                  className="border-t border-[#2C2421]/5 bg-[#FAF8F5]/40 overflow-hidden"
                >
                      ...
                      <button
                        ...
                        className={`py-3 px-2 rounded-xl text-center transition-custom hover-scale text-xs font-bold ${
                          customization.size === s.id
                            ? "bg-[#2B4D3D] text-[#F4F0EB]"
                            : "bg-[#FAF8F5] text-[#2C2421] border border-[#2C2421]/10 hover:border-[#2B4D3D]"
                        }`}
                      >
          ...
          <div className="rounded-[24px] bg-[#FAF8F5] border border-[#2C2421]/5 shadow-sm overflow-hidden transition-all duration-300">
            ...
            >
              ...
              <ChevronDown
                size={18}
                className={`text-[#2C2421]/40 transition-transform duration-300 ${
                  expandedSection === "milk" ? "rotate-180 text-[#2B4D3D]" : ""
                }`}
              />
            </button>
          ...
          <div className="rounded-[24px] bg-[#FAF8F5] border border-[#2C2421]/5 shadow-sm overflow-hidden transition-all duration-300">
            ...
            >
              ...
              <ChevronDown
                size={18}
                className={`text-[#2C2421]/40 transition-transform duration-300 ${
                  expandedSection === "sweetener" ? "rotate-180 text-[#2B4D3D]" : ""
                }`}
              />
            </button>
          ...
          <div className="rounded-[24px] bg-[#FAF8F5] border border-[#2C2421]/5 shadow-sm overflow-hidden transition-all duration-300">
            ...
            >
              ...
              <ChevronDown
                size={18}
                className={`text-[#2C2421]/40 transition-transform duration-300 ${
                  expandedSection === "topping" ? "rotate-180 text-[#2B4D3D]" : ""
                }`}
              />
            </button>
          ...
          <div className="rounded-[24px] bg-[#FAF8F5] border border-[#2C2421]/5 shadow-sm overflow-hidden transition-all duration-300">
            ...
            >
              ...
              <ChevronDown
                size={18}
                className={`text-[#2C2421]/40 transition-transform duration-300 ${
                  expandedSection === "ice" ? "rotate-180 text-[#2B4D3D]" : ""
                }`}
              />
            </button>
